...
class CommercialController
{
    ...
    public function add()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($this->validator->verif('users', 'telephone_user', $telephone)) {
                $msg = ['msg' => 'Ce numéro de téléphone existe déjà!', 'status' => 0];
                echo json_encode($msg);
                return;
            }
            // Vérification si l'email existe déjà
            if (!empty($email) && $this->validator->verif('users', 'email_user', $email)) {
    ...
    public function edit()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($this->validator->_verif('users', 'telephone_user', $telephone, 'id_user', $id)) {
                $msg = ['msg' => 'Ce numéro de téléphone est déjà utilisé!', 'status' => 0];
                echo json_encode($msg);
                return;
            }
            // Vérification si l'email existe déjà pour un autre utilisateur
            if (!empty($email) && $this->validator->_verif('users', 'email_user', $email, 'id_user', $id)) {
